...
public class ActionSelectionScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new Button(this.width / 2 - 100, y - 30, 200, 20, Component.translatable("button.keybindsgalore.remove_priority"), button -> {
            removeCallback.run();
            Minecraft.getInstance().setScreen(parent);
        }));

        for (KeyMapping kb : options) {
            this.addRenderableWidget(new Button(this.width / 2 - 100, y, 200, 20, Component.translatable(kb.getCategory()).append(": ").append(Component.translatable(kb.getName())), button -> {
                callback.accept(kb);
                Minecraft.getInstance().setScreen(parent);
            }));
            y += 25;
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredString(poseStack, this.font, this.title, this.width / 2, 20, 0xFFFFFFFF);
    }
